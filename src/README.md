...
Scheduler (edge MQTT simulator) quick notes (logically isolated):
...
All devices receive all the messages. They have an internal filter. Messages to devices must be `{"device_id":"<DEVICE_ID>","op":"power","value":"<on/off>"}`
